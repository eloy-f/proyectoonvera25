...
const usuarioService = __importStar(require("../services/usuarios.service"));
const base_response_1 = require("../shared/base-response");
const verificarUsuario = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const data = req.body;
        const usuario = yield usuarioService.verificarUsuario(data);
        if (!usuario) {
            res.status(404).json(base_response_1.BaseResponse.error('No se encontrarion datos'));
            return;
        }
    }
    catch (error) {
